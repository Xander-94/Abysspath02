...
## 项目简介
AbyssPath 是一个基于 Flutter 开发的移动应用，结合 AI 技术为用户提供个性化的学习路径规划。应用通过评估用户当前知识水平和学习目标，生成定制化的学习计划。（项目名称已更新）
...
3. 学习路径
   ...
   - **知识图谱可视化**:
     - **实现**: 前端使用 `graphview` 包结合 `InteractiveViewer` 实现学习路径的交互式图谱可视化 (`KnowledgeGraphView`)。
     - **多布局支持**:
       - **树形布局 (`BuchheimWalkerAlgorithm`)**: 默认展示最大的连通组件，适合查看清晰的层级结构。此模式下节点不可拖动，以维持结构稳定性。
       - **力导向布局 (`FruchtermanReingoldAlgorithm`)**: 展示所有节点和边，节点间通过模拟物理力自然分布，适合探索关系和自由组织视图。此模式下节点可以自由拖动。
       - **用户切换**: 视图提供控件允许用户在两种布局间自由切换。
     - **交互**: 支持缩放、平移、重置视图、节点点击交互。
   ...
   - **重要开发提示**:
     ...
     - **图谱布局调试**: 树形布局严格要求单一树结构，对于包含多个独立组件的图，仅显示最大组件。力导向布局更适合非树状结构。
...
4. 个人中心与设置
   ...
   - 密码修改
   - 头像管理
   - **设置页优化**: 移除了未实现功能（深色模式、语言），添加了设置项分组，优化了退出登录流程（增加确认对话框）。

5. 首页
   - **UI/UX 优化**:
     - 使用 `select` 优化 Riverpod 状态监听，减少不必要重建。
     - 将核心区块拆分为独立 Widget 提升性能和可维护性。
     - 改进了加载状态显示（可考虑 Shimmer 特效）和错误提示。
     - 优化了学习进度条显示。
...
- v0.3.0 (当前版本)
  ...
  - **知识图谱可视化**: 使用 `graphview` 包实现交互式图谱展示，**支持树形和力导向两种布局切换**。
  ...
  - **UI 优化**: 
    ...
    - **优化首页**: 提升状态管理性能，改进 UI 元素（如进度条）。
    - **优化设置页**: 移除未实现功能，增加分组，改进退出登录交互。
  - **项目重命名**: 项目名称更新为 AbyssPath。
